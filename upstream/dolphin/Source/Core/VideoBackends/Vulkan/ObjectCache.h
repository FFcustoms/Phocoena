...
public:
  ...
  // VkPipelineCache is externally synchronized while Phocoena's single
  // background shader worker is active. Render-pass cache mutation uses a
  // separate mutex so pipeline creation can safely query it first.
  std::mutex& GetPipelineCacheMutex() { return m_pipeline_cache_mutex; }
#endif
